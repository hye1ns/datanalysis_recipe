...
save_review_fname = './data/recipe_review_5.csv'
save_recipe_fname = './data/recipe_main_5.csv'
    ...
    for type_key, type_value in by_type.items():
        for situ_key, situ_value in by_situation.items():
            if situ_key == '초스피드':
                ...
                print('review 길이: ', len(list4rdf))
                print('밑반찬, 일상 종료.')
                exit()
            for ing_key, ing_value in by_ingredient.items():
                if type_key=='밑반찬' and situ_key=='일상':
                    if ing_key=='소고기' or ing_key=='돼지고기' or ing_key=='닭고기' or ing_key=='육류' or ing_key=='채소류' or ing_key=='해물류' or ing_key=='달걀/유제품' or ing_key=='쌀' or ing_key=='밀가루' or ing_key=='건어물류':
                        continue
                for method_key, method_value in by_method.items():
                    if ing_key=='버섯류':
                        if method_key=='볶음' or method_key=='끓이기' or method_key=='부침' or method_key=='조림' or method_key=='무침' or method_key=='비빔' or method_key=='찜' or method_key=='절임':
                            continue
